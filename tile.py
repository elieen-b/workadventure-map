...
def jsonFromPolygon(polygon, params=(0.000000,0.001000,0.005000)):
  ...
  urllib.request.urlopen(polygonUrl).read()
  if params is None:
    url = "http://polygons.openstreetmap.fr/get_geojson.py?id={polygon}&params=0".format(polygon=polygon)
  else:
    query = 'x={x:.6f}&y={y:.6f}&z={z:.6f}&generate=Submit+Query'.format(x=params[0], y=params[1], z=params[2])
    req = urllib.request.Request(url=polygonUrl, data=query.encode('utf-8'))
    urllib.request.urlopen(req).read() # throw away it is only human readable HTML
...
def jsonFromUrl(geoJsonUrl):
  geoJson = urllib.request.urlopen(geoJsonUrl).read()
